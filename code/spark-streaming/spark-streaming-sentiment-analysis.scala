// :load  /mnt/hgfs/f/code/spark-streaming/spark-streaming-sentiment-analysis.scala
...
val probRDD  	= sc.textFile("/user/cloudera/sentiment/sentiment_bag.csv").map(s=>s.split(",")).map(s=> (s(0), (s(1).toFloat, s(2).toFloat)  ))
...
val filters = new Array[String] (7)
	filters(0) = "Ted 2"
	filters(1) = "Tami-Lynn"
	filters(2) = "Tami Lynn"
