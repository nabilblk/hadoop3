...
var ipRDD = htmlRDD.map(line => (line.split(' ')(0),line.split(' ')(2)))   //The IP RDD is of type <k,v>
ipRDD.take(5).foreach(t => println(t._1 + "/" + t._2))

var ipRDD = htmlRDD.map(line => (line.split(' ')(0),line.split(' ')(2), line.split(' ')(3) ))
...
splitRDD.take(1).foreach(t=>println(t(5)))n
